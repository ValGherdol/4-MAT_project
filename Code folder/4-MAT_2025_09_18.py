...
if 1 in L_steps :
    ...
    if create == False : # Load a preexisting network
        PCEIN_net = PCEIN.loadPearsonCoExpressionIntersectedNetwork(file_name_1a,annonce=10000)
        with open(Log_file,'a') as file : file.write(f"Network loaded.\n")
    
    else : # Build a new network
        if D_args['Anchor centered P-CEIN'] : a_L = L_Final_anchors
        else : a_L = None
        P_t = D_args['Pearson principal threshold']
        S_tps = D_args['Research time announcement P-CEIN']
        ...
        PCEIN_net = PCEIN.buildPearsonCoExpressionIntersectedNetwork(dataframe_list=L_DF_3TP_min , gene_list=L_pool ,
                                                                 anchor_list=a_L ,threshold_Pearson=P_t , search_tps=S_tps)
        print(f"Initial network composition : {len(PCEIN_net.nodes)} nodes and {len(PCEIN_net.edges)} edges")
        print("----------------------------")

        # Initial new graph saving
        L_nodes = list(PCEIN_net.nodes) ; L_edges = list(PCEIN_net.edges)
        PCEIN.saveNetwork(PCEIN_net , anchor_lists=L_L_anchors , color_list=L_colors , file_name=file_name_1a)
        with open(Log_file,'a') as file : file.write(f"Full network built and saved : {len(PCEIN_net.nodes)} genes and {len(PCEIN_net.edges)} associations.\n")

        # Initial new graph's general data saving
        df_data = PCEIN.saveNetworkData(PCEIN_net , L_anchors_labels=L_labels , L_anchors_list=L_L_anchors , dataframe_file=f"{folder_name}Result_1a_{etape}_data.csv")
        del(df_data)
    ...
    if D_args['Pearson dynamic threshold'] :
        ...
        PCEIN_net , dynamic = PCEIN.dynamic_filtering(PCEIN_net , factor=D_args['Dynamic threshold factor'])
        ...
        print(f"Dynamically filtered network composition : {len(PCEIN_net.nodes)} nodes and {len(PCEIN_net.edges)} edges")
        print("----------------------------")

        # Dynamically filtered graph saving
        file_name_1b = f"{folder_name}Result_1b_{etape}_dynamic_graph.txt"
        L_nodes = list(PCEIN_net.nodes) ; L_edges = list(PCEIN_net.edges)
        PCEIN.saveNetwork(PCEIN_net , anchor_lists=L_L_anchors , color_list=L_colors , file_name=file_name_1b)
        with open(Log_file,'a') as file : file.write(f"Dynamic Pearson network saved : {len(PCEIN_net.nodes)} genes and {len(PCEIN_net.edges)} associations.\n")

        # Dynamically filtered graph's general data saving
        df_data = PCEIN.saveNetworkData(PCEIN_net , L_anchors_labels=L_labels , L_anchors_list=L_L_anchors , dataframe_file=f"{folder_name}Result_1b_{etape}_dynamic_data.csv")
        del(df_data)
    ...
    if D_args['Minimum neighborhood P-CEIN'] != 'default' :
        ...
        PCEIN_net = PCEIN.neighborhood_filtering(PCEIN_net , anchor_lists=L_L_anchors , limite_list=D_args['Minimum neighborhood P-CEIN'])
        print(f"Neighbor filtered network composition : {len(PCEIN_net.nodes)} nodes and {len(PCEIN_net.edges)} edges")
        print("----------------------------")

        # Anchor-neighborhood filtered graph saving
        file_name_1c = f"{folder_name}Result_1c_{etape}_relevant_neighborhood_graph.txt"
        L_nodes = list(PCEIN_net.nodes) ; L_edges = list(PCEIN_net.edges)
        PCEIN.saveNetwork(PCEIN_net , anchor_lists=L_L_anchors , color_list=L_colors , file_name=file_name_1c)
        with open(Log_file,'a') as file : file.write(f"Relevant neighborhood network saved : {len(PCEIN_net.nodes)} genes and {len(PCEIN_net.edges)} associations.\n")

        # Anchor-neighborhood filtered graph's general data saving
        df_data = PCEIN.saveNetworkData(PCEIN_net , L_anchors_classes=L_labels , L_anchors_list=L_L_anchors , dataframe_file=f"{folder_name}Result_1c_{etape}_relevant_neighborhood_data.csv")
        del(df_data)
...
#                    print(f"{n1} ({L_labels[i]}) : {len(PCEIN_net[n1])}")
                    if n1 in PCEIN_net.nodes :
                        for n2 in PCEIN_net[n1] : D_assoc[(n1,L_labels[i])][etape].append((n2,PCEIN_net[n1][n2]['weight']*PCEIN_net[n1][n2]['sign']))
                        break
#                    print("----------")
        ...
        for n1 in L_pool :
            ...
            if n1 in PCEIN_net.nodes :
                for n2 in PCEIN_net[n1] : D_assoc[(n1,'Candidate')][etape].append((n2,PCEIN_net[n1][n2]['weight']*PCEIN_net[n1][n2]['sign']))
    del(PCEIN_net)
...
if 2 in L_steps :
    ...
    L_RBH = [] ; version = D_args['KNN version'] ; rbh = D_args['RBH sub-step']
    neigh = D_args['Number of neighbors'] ; factor = D_args['Threshold factor KNN_2']
    if type(factor) != list : # If a single dynamic factor is provided
        for i,df in enumerate(L_DF) :
            print(f"{etape} analysis on dataset {i+1}")
...
#    print(len(RBH_inter))
    ...
    if version == 'KNN_1' : # KNN version 1 : only the neighbors are known
        ...
        for n1,voisins in RBH_inter.items():
            for n2 in voisins :
                if not KNN_net.has_edge(n1,n2) : KNN_net.add_edge(n1,n2)

    else : # KNN version 2 : each neighbor of a gene knows the distance between them
        KNN_net = nx.Graph()
        for n1,voisins in RBH_inter.items():
            for (n2,dist) in voisins :
                if not KNN_net.has_edge(n1,n2) : KNN_net.add_edge(n1,n2,weight=dist)
    print(f"Initial network composition : {len(KNN_net.nodes)} nodes and {len(KNN_net.edges)} edges")
    print("---------------")

    with open(Log_file,'a') as file : file.write(f"Initial network built : {len(KNN_net.nodes)} genes and {len(KNN_net.edges)} associations.\n")
    ...
    if D_args['Anchor Genes lists'] != []:
        ...
        KNN_net = KNN_RBH.cleanGraph(KNN_net , anchor_lists=L_L_anchors)

        print(f"Filtered network composition : {len(KNN_net.nodes)} nodes and {len(KNN_net.edges)} edges")
        print("---------------")
        with open(Log_file,'a') as file : file.write(f"Network filtered : {len(KNN_net.nodes)} genes and {len(KNN_net.edges)} associations.\n")

    ### Graph saving
    file_name_2 = f"{folder_name}Result_2_{etape}_Graph.txt"
    KNN_RBH.saveNetwork(KNN_net , method=version , anchor_lists=L_L_anchors , color_list=L_colors , file_name=file_name_2)
    ...
    ### For each anchor, we save it's neighbors and the euclidean distance between them (average distance if several datasets)
    if D_args['Anchor Genes lists'] != [] : # If anchors are provided
        for gene,assoc in D_assoc.items():
            D_assoc[gene][etape] = []
            n1 = gene[0]
#            print(n1,assoc)
            if n1 in KNN_net.nodes : 
                if version == 'KNN_1' : D_assoc[gene][etape] = list(KNN_net[n1])
                else :
                    for n2 in list(KNN_net[n1]) : D_assoc[gene][etape].append((n2,KNN_net[n1][n2]['weight']))
...
if 3 in L_steps :
    ...
    for a,df_norm in enumerate(L_DF_Norm) : # For each normalized dataset
        ...
        distancesKNN = nbrsTestDist ; indicesKNN = nbrsTestIdx
        ...
        for b,L_AnchorIdx in enumerate(L_L_AnchorIdx) : # For each anchor genes list
            ...
            NPC_net_1, adj = NPC.createGraph(indicesKNN, distancesKNN, specialGenesIndex, geneIndex)
            print(f"Network built ({len(NPC_net_1.nodes)} nodes & {len(NPC_net_1.edges)} edges)")

            # Deletion of edges with too high distances
            NPC_net_1 = NPC.cleanGraphDistance(NPC_net_1, distancesKNN, factor=factor, version='Bilateral')#np.mean(distancesKNN)+2*(np.std(distancesKNN)))
            print(f"Graph clean Dist ({len(NPC_net_1.nodes)} nodes & {len(NPC_net_1.edges)} edges)")

            # Deletion of nodes with with too few anchor neighbors (and their remaining edges)
            if MinNB == 'Dynamic' :
                NPC_net_1 , MinNB = NPC.dynamicCleanGraph(NPC_net_1, specialGenesIndex)
                with open(Log_file,'a') as file : file.write(f"Dataset {a+1} dynamic minimum of {list_labels[b]} neighbors : {MinNB}.\n")
            else : NPC_net_1 = NPC.cleanGraph(NPC_net_1, MinNB)
            print(f"Graph clean Nodes ({len(NPC_net_1.nodes)} nodes & {len(NPC_net_1.edges)} edges) : {Set_fnc.time_count(time.time()-step_time)}")
            
            # Ranking matrix filling
            rankMat = NPC.rankingNodes(NPC_net_1, specialGenesIndex)
            print(f"Ranking matrix filled ({len(rankMat)}) : {Set_fnc.time_count(time.time()-step_time)}")
            del(NPC_net_1)
            #-------------------------------------------
            to_remove = []
...
#            print(rankMat_str)
    ...
    ### From a dataset to another, the linked anchor gene may be different.
...
#        print(f"{L_Cands[i]}")
        ...
        print("\n----------------------\n")
    print(f"Inter Multi-Anchors candidates : {len(Inter_all)}")
#    print(f"{Inter_all}")
    ...
    for i,inter in enumerate(L_Cands) :
        for n1 in list(inter.keys()) :
            NPC_net_2.add_node(n1)
            for k,v in L_dico_N[i][n1].items():
                if type(v) == list :
                    for n2 in v :
                        if (n2 not in NPC_net_2.nodes) : NPC_net_2.add_node(n2)
                        NPC_net_2.add_edge(n1,n2,weight=k)
    print(f"Final network composition : {len(NPC_net_2.nodes)} nodes and {len(NPC_net_2.edges)} edges")
    print("---------------")
    
    graph_file_3c = f"{folder_name}Result_3c_{etape}_graph.txt"
    NPC.saveNetwork(NPC_net_2 , anchor_lists=L_L_anchors , color_list=L_colors , file_name=graph_file_3c)
    with open(Log_file,'a') as file : file.write(f"Network built and saved : {len(NPC_net_2.nodes)} genes and {len(NPC_net_2.edges)} associations.\n")
    ...
    for gene,assoc in D_assoc.items():
        ...
        n1 = gene[0]
        if n1 not in NPC_net_2.nodes : continue
        else : 
            for n2 in NPC_net_2[n1] : 
                w = NPC_net_2[n1][n2]['weight']
...
if 4 in L_steps :
    ...
    nb_clust = D_args['Number of clusters'] ; nb_deviation = D_args['Maximum deviation']

    LD_commonPath = [] ; L_path_length = []
    for i,df in enumerate(L_DF) : # For each dataset
        print(f"Cluster pathing for dataset {i+1}")
        L_colonnes = list(df.keys())[1::]
        n_tp = len(L_colonnes)
        L_path_length.append(n_tp)
        
        # Clustering calculation
        L_dico = []
        for j in range(n_tp): L_dico.append(CP.KMeans_1D(df , j , n_clusters=nb_clust))
        ...
        print(f"Number of paths calculated : {len(D_commonPath)}")
        print("-----------")

    with open(Log_file,'a') as file : file.write(f"All datasets analyzed.\n")

    # Checking the max number of allowed deviations for each sub-path
    # The max number of allowed deviations can't be equal nor higher that the length of it's respective path
    # (at least one timepoint of a sub-path must be equal between two genes)
    if type(nb_deviation) == list :
        for i,dev in enumerate(nb_deviation) : nb_deviation[i] = min(dev , L_path_length[i]-1)
    else : nb_deviation = min(nb_deviation , sum(L_path_length)-1)
    ...
    file_name_4a = f"{folder_name}Result_4a_{etape}_PathList.txt"
    ...
    # Sort couples of genes who follow paths with an allowed number of differences
    if type(nb_deviation) == list :
        with open(Log_file,'a') as file :
            line = "Numbers of allowed deviations for each sub-path :"
            for i,dev in enumerate(nb_deviation) : line += f" {dev}/{L_path_length[i]},"
            file.write(f"{line[0:-1]}.\n")
            file.write(f"Total number of allowed path deviations : {sum(nb_deviation)}/{sum(L_path_length)}.\n")
    else :
        with open(Log_file,'a') as file : file.write(f"Total number of allowed path deviations : {nb_deviation}/{sum(L_path_length)}.\n")
    if D_args['Anchor centered CP'] : a_L = L_Final_anchors
    else : a_L = None
    S_tps = D_args['Research time announcement CP']
    D_deviantPaths = CP.deviationPathing(D_geneRegroupedPaths , anchor_list=a_L , Max_deviation=nb_deviation , path_lengths=L_path_length , search_tps=S_tps)

    # Saving the deviated paths and the genes that follow them
    file_name_4b = f"{folder_name}Result_4b_{etape}_DeviantPathsList.txt"
    CP.saveDeviantPaths(D_deviantPaths , file_name=file_name_4b)
    with open(Log_file,'a') as file : file.write(f"Lists of gene couples with deviant paths saved : {len(D_deviantPaths)} paths found.\n")

    # Building the neighborhood network and saving it
    CP_net = nx.Graph()
    for key in sorted(D_deviantPaths, key=lambda key: len(D_deviantPaths[key]), reverse=True):
        (path,match_ratio) = key
        L_couples = D_deviantPaths[(path,match_ratio)]
        for i,(n1,n2) in enumerate(L_couples) :
            if n1 not in list(CP_net.nodes) : CP_net.add_node(n1)
            if n2 not in list(CP_net.nodes) : CP_net.add_node(n2)
            CP_net.add_edge(n1,n2,path=path,ratio=match_ratio)
    print(f"Final network composition : {len(CP_net.nodes)} nodes and {len(CP_net.edges)} edges")
    print("---------------")

    graph_file_4c = f"{folder_name}Result_4c_{etape}_graph.txt"
    CP.saveNetwork(CP_net , anchor_lists=L_L_anchors , color_list=L_colors , file_name=graph_file_4c)

    with open(Log_file,'a') as file : file.write(f"Network built and saved : {len(CP_net.nodes)} genes and {len(CP_net.edges)} associations.\n")
    
    ### Saving of Cluster Path associations in Linkage dictionary for consensus
    ### For each anchor, we save it's neighbors and the match ratio of the path followed by the two genes.
    if D_args['Anchor Genes lists'] != [] : # If there are anchors
        for i,L_anchors in enumerate(L_L_anchors):
                for n1 in L_anchors :
                    D_assoc[(n1,L_labels[i])][etape] = []
                    if n1 in CP_net.nodes :
                        for n2 in CP_net[n1] :
                            D_assoc[(n1,L_labels[i])][etape].append((n2,CP_net[n1][n2]['ratio']))
    else : # If there are no anchors
        for n1 in L_pool :
            D_assoc[(n1,'Candidate')][etape] = []
            if n1 in CP_net.nodes :
                for n2 in CP_net[n1] : D_assoc[(n1,L_labels[i])][etape].append((n2,CP_net[n1][n2]['ratio']))
...
#    print(D_repartition)
    ...
    Consensus_net , L_anchors , L_candidates , D_anchors = Cons_fnc.buildConsensusNetwork(D_assoc , D_steps_names)

    if L_L_anchors != [] : # If anchors have been provided
        print(f"Total genes : {len(Consensus_net.nodes)} / {len(L_anchors)} anchors / {len(L_candidates)} candidates")
    else : # If no anchor has been provided
        print(f"Total genes : {len(Consensus_net.nodes)}")
    print(f"Total edges : {len(Consensus_net.edges)}")
    print("----------------------------")
    with open(Log_file,'a') as file : file.write(f"Consensus network built : {len(Consensus_net.nodes)} genes and {len(Consensus_net.edges)} associations.\n")

    graph_file_END = f"{folder_name}Result_END_graph.txt"
    Cons_fnc.saveNetwork(Consensus_net , L_candidates , D_anchors , anchor_lists=L_L_anchors ,

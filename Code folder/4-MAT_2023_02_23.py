...
if 4 in L_steps :
    ...
    nb_deviation = D_args['Maximum deviation']
    ...
    for key in sorted(D_deviantPaths, key=lambda key: len(D_deviantPaths[key]), reverse=True):
        ...
        for i,(n1,n2) in enumerate(L_couples) :
            ...
            G.add_edge(n1,n2,path=path,ratio=match_ratio)
    print(f"Final network composition : {len(G.nodes)} nodes and {len(G.edges)} edges")
    print("---------------")

    graph_file_4c = f"{folder_name}Result_4c_{etape}_graph.txt"
    CP.saveNetwork(G , anchor_lists=L_L_anchors , color_list=L_colors , file_name=graph_file_4c)
    ...
    if D_args['Anchor Genes lists'] != [] : # If there are anchors
        for i,L_anchors in enumerate(L_L_anchors):
                for n1 in L_anchors :
                    ...
                    if n1 in G.nodes :
                        for n2 in G[n1] :
                            D_assoc[(n1,L_labels[i])][etape].append((n2,G[n1][n2]['ratio']))
    else : # If there are no anchors
        for n1 in L_pool :
            D_assoc[(n1,'Candidate')][etape] = []
            if n1 in G.nodes :
                for n2 in G[n1] : D_assoc[(n1,L_labels[i])][etape].append((n2,G[n1][n2]['ratio']))

...
import pickle
import Stage_functions as fnc
import CVI_functions as CVI
import warnings
        ...
        for b,L_AnchorIdx in enumerate(L_L_AnchorIdx) :
            ...
            for NN in NN_arr:
                ...
                for distance in distances:
                    ...
                    G, adj = CVI.createGraph(indicesKNN, distancesKNN, specialGenesIndex, geneIndex)

#                    print(np.mean(distancesKNN))
#                    print(np.std(distancesKNN))

                    G = CVI.cleanGraphDistance(G, np.mean(distancesKNN)+(np.std(distancesKNN)/2.0))

                    G = CVI.cleanGraph(G, MinNB)
                    adj = CVI.getBlueNodes(G, specialGenesIndex)

                    G = CVI.completeGraphMin(G, adj, indicesKNN, distancesKNN)
                    rnodes = CVI.getBlueNodes(G, specialGenesIndex)

                    rankMat = CVI.rankingNodes(G, specialGenesIndex) # The idea is to rank genes and give the top 10
                    aux = []
...
#            print(line[0:-1])
    ...
    with open(Log_file,'a') as file : file.write(f"Common path gene lists saved.\n")

...
if 1 in L_etapes :
    ...
    if create : # Build a new network
        ...
        RICEP = fnc.ThresholdedCrossNetworkConstancy_5(dataframe_list=L_DF , gene_list=L_pool , anchor_list=a_L ,
                                                       ...
                                                       search_tps=S_tps , global_tps=G_tps)
        ...
        df_data = fnc.save_PearsonGraph_data_2(RICEP , L_anchors_labels=L_labels , L_anchors_list=L_L_anchors , dataframe_file=f"Result_1a_{etape}_data.csv")
        del(df_data)
        with open(Log_file,'a') as file : file.write(f"Full network data saved.\n")

    else : # Load a preexisting network
        RICEP = fnc.load_PearsonGraph_2(file_name,annonce=10000)
    ...
    if D_args['Pearson dynamic threshold'] :
        ...
        with open(f"Result_1b_{etape}_dynamic_graph.txt",'w') as file :
            for i,node in enumerate(L_nodes) :
                cand = True
                for j,L_anchors in enumerate(L_L_anchors):
                    if node in L_anchors :
        ...
        df_data = fnc.save_PearsonGraph_data_2(RICEP , L_anchors_classes=L_labels , L_anchors_list=L_L_anchors , dataframe_file=f"Result_1b_{etape}_dynamic_data.csv")
        del(df_data)
...
if 3 in L_etapes :
    ...
    min_max_scaler = preprocessing.MinMaxScaler() ### Fixed normalization for each timepoint
    L_DF_Norm = []
    ...
    for a,df_norm in enumerate(L_DF_Norm) : # For each dataset
        ...
        for b,L_AnchorIdx in enumerate(L_L_AnchorIdx) : # For each anchor genes list
            ...
            if MinNB == 'Dynamic' :
                G , MinNB = NPC.dynamicCleanGraph_V(G, specialGenesIndex)
            ...
            rankMat = NPC.rankingNodes_V(G, specialGenesIndex)
...
#            print(file_name)
    ...
    lists_file_name = f"Result_3t_{etape}_CandidateLists_file.txt"
...
if 4 in L_etapes :
    ...
    for name in L_pool : # For each gene
        D_names[name] = []
        for dico in LD_vecteurs : # For each dictionary of "path - genes list"
            for k,v in dico.items():
                if name in v :
    ...
    D_groupes = {}
    for gene,path in D_names.items():
#        print(gene,path)
        if len(path) == 1 : key = path[0]
        else : key = tuple(path)
        if key not in D_groupes.keys(): D_groupes[key] = [gene]
        else : D_groupes[key].append(gene)
...
#            print(path,groupe)
#            print(path)
            line = f">Path:{path} ; {len(groupe)} total genes"
...
#            print(line[0:-1])
    ...
    with open(graph_file,'w') as file :
        for i,node in enumerate(list(G.nodes)) : file.write(f">{node}:{CN_map[i]}\n")
        for i,(n1,n2) in enumerate(list(G.edges)) : file.write(f"{n1} ; {n2} ; path:{str(G[n1][n2]['weight'])}\n")
